...
nb_lanes = 4
pp = PhysicalParam(nb_lanes,lane_length=100.) #2.=>col_length=8   #Sets parameters of lanes and cars. Quite a few standard parameters are set here.
_discount = 1.
nb_cars=10

rmodel = NoCrashRewardModel()   #Sets parameters of reward model
dmodel = NoCrashIDMMOBILModel(nb_cars, pp)   #Sets up simulation model parameters.
dmodel.max_dist = 10000   #Make svisualization fail if max_dist is set to the default Inf
...
s = initial_state(mdp::NoCrashMDP, rng) #Creates inital state by first initializing only ego vehicle and then running simulatio for 200 steps, where additional vehicles are randomly added.
# @show s.cars[1]
#visualize(mdp,s,MLAction(0,0))
write_to_png(visualize(mdp,s,0.0),"Figs/initState.png")

policy = Multilane.BehaviorPolicy(mdp, Multilane.NORMAL, false, rng)   #Sets up behavior of ego vehicle for the simulation. False referes to that lane changes are allowed.
