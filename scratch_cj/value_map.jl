...
for x in 0:1.:1000
    ...
    for y in 1:0.5:4
        if mod(y,1.0)==0.
            s_ego_veh = CarState(s.cars[1].x, y, 25.0, 0.0, s_ego_veh.behavior, s_ego_veh.length, s_ego_veh.width, s_ego_veh.id)
            s.cars[1] = s_ego_veh
            s.x = x
            push!(s_vec,deepcopy(s))
        else
            for lane_change in [-1,1.]
                s_ego_veh = CarState(s.cars[1].x, y, 25.0, lane_change, s_ego_veh.behavior, s_ego_veh.length, s_ego_veh.width, s_ego_veh.id)
                s.cars[1] = s_ego_veh
                s.x = x
                push!(s_vec,deepcopy(s))
            end
